...
def start_system(jar_path, basePort):
	...
	proc = {'registry': subprocess.Popen(["java", "-jar", jar_path, "registry", str(port['registry'])])}
	reg_uri = "http://127.0.0.1:"+str(port['registry'])
	time.sleep(5)
	for i in range(len(msnames)):
		port[msnames[i]] = basePort+i+1
		proc[msnames[i]] = subprocess.Popen(["java", "-jar", jar_path, msnames[i], reg_uri, str(port[msnames[i]])])
